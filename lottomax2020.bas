...
REM
REM *** Last Update : January 01 2020 - 02:29 AM     Initial conversion
REM
...
LOCATE 11, 14
PRINT "Release Date  : Jan 01  2020 -  01:00 AM           "
LOCATE 12, 14
PRINT "Registered To :                                    "
LOCATE 12, 30
PRINT reg$
...
IF dayflag = 0 THEN
    IF day1 = 6 THEN day8 = 6
    IF day1 = 5 THEN day8 = 6
    IF day1 = 4 THEN day8 = 6
    IF day1 = 3 THEN day8 = 6
    IF day1 = 2 THEN day8 = 6
    IF day1 = 1 THEN day8 = 6
    IF day1 = 0 THEN day8 = 6
ELSEIF dayflag = 1 THEN
    IF day1 = 6 THEN day8 = 6
    IF day1 = 5 THEN day8 = 6
    IF day1 = 4 THEN day8 = 6
    IF day1 = 3 THEN day8 = 6
...
IF day1 = 6 AND count = -1 THEN
    ...
    END IF
ELSEIF day1 = 6 AND count = 1 THEN
    IF rollover = 0 THEN
        adddays = 7
        dy = dy + adddays
    ELSEIF rollover = 1 THEN
...
ELSEIF day1 = 6 AND count = 0 THEN
    IF rollover = 0 THEN
        adddays = 7
        dy = dy + adddays
    ELSEIF rollover = 1 THEN
...
IF y = 2018 THEN
    ...
    IF dy > 31 AND m = 3 THEN m = 4: dy = 6: rollover = 1: adjust = 0
    IF dy > 30 AND m = 4 THEN m = 5: dy = 4: rollover = 1: adjust = 1
    IF dy > 31 AND m = 5 THEN m = 6: dy = 1: rollover = 1: adjust = 0
    IF dy > 30 AND m = 6 THEN m = 7: dy = 6: rollover = 1: adjust = 1
    IF dy > 31 AND m = 7 THEN m = 8: dy = 3: rollover = 1: adjust = 0
    IF dy > 31 AND m = 8 THEN m = 9: dy = 7: rollover = 1: adjust = 0
    IF dy > 30 AND m = 9 THEN m = 10: dy = 5: rollover = 1: adjust = 1
    IF dy > 31 AND m = 10 THEN m = 11: dy = 2: rollover = 1: adjust = 0
    IF dy > 30 AND m = 11 THEN m = 12: dy = 7: rollover = 1: adjust = 1
    IF dy > 31 AND m = 12 THEN y = 2020: m = 12: dy = 4: rollover = 1: adjust = 0
END IF
IF y = 2019 THEN
    IF dy > 31 AND m = 1 THEN m = 2: dy = 1: rollover = 1: adjust = 0
    IF dy > 28 AND m = 2 THEN m = 3: dy = 1: rollover = 1: adjust = 3
    IF dy > 31 AND m = 3 THEN m = 4: dy = 5: rollover = 1: adjust = 0
    ...
    IF dy > 31 AND m = 12 THEN y = 2020: m = 12: dy = 3: rollover = 1: adjust = 0
END IF
IF y = 2020 THEN
...
IF y = 2021 THEN
    ...
    IF dy > 31 AND m = 10 THEN m = 11: dy = 3: rollover = 1: adjust = 0
    IF dy > 30 AND m = 11 THEN m = 12: dy = 1: rollover = 1: adjust = 1
    IF dy > 31 AND m = 12 THEN y = 2022: m = 1: dy = 1: rollover = 1: adjust = 0
END IF
IF y = 2022 THEN
    IF dy > 31 AND m = 1 THEN m = 2: dy = 2: rollover = 1: adjust = 0
    IF dy > 28 AND m = 2 THEN m = 3: dy = 2: rollover = 1: adjust = 3
...
PRINT
money:
INPUT "How many sets of tickets do you wish to purchase  [At $5.00 per 3 sets of numbers : ]  $", sets
IF sets < 4 THEN
    PRINT "You need to spend at least $5.00 - Please try again!"
    GOTO money
END IF
sets = INT(sets * 5)
PRINT
...
PRINT USING "  Winning Numbers Database Updated On :  \ \  \ \ ##-#### "; dayofweek$(day1); month$(m), dy, y
IF crs$ = "Y" THEN COLOR 10, 5
LOCATE 5, 8
...
ELSEIF answer$ = "%" THEN
    ...
    distribute$ = "QB64.ORG"
    serial$ = reg2$
    version$ = "Lotto Max RC4"
    release$ = "January 01 2020 - 03:00 AM"
    pass$ = "Password"
    IF pass2$ = pass$ THEN
        CLS
        PRINT
...
END IF
FOR i = 1 TO amount
    IF bonus$ = "Y" AND numb(i) < low THEN
        outpt1(i) = numb(i) + bonus(i)
    ELSEIF bonus$ = "N" AND numb(i) < low THEN
        outpt1(i) = numb(i)
    END IF
NEXT i
PRINT
...
FOR j = 1 TO 10
    ...
    PRINT USING "  Num ## : ###"; i + 10; outpt1(j + 10);
...
END IF
FOR i = 1 TO amount
    IF bonus$ = "Y" AND numb(i) > high THEN
        outpt2(i) = numb(i) + bonus(i)
    ELSEIF bonus$ = "N" AND numb(i) > high THEN
        outpt2(i) = numb(i)
    END IF
NEXT i
PRINT
...
IF hardcopy$ = "Y" THEN
    ...
    LPRINT
    FOR i = 1 TO 10
        LPRINT USING "Num ## : ###"; i; outpt(j);
        LPRINT USING "  Num ## : ###"; i + 10; outpt(j + 10);
        LPRINT USING "  Num ## : ###"; i + 20; outpt(j + 20);
        LPRINT USING "  Num ## : ###"; i + 30; outpt(j + 30);
        LPRINT USING "  Num ## : ###"; i + 40; outpt(j + 40)
    NEXT i
    LPRINT
    LPRINT
    ...
    LPRINT
    FOR i = 1 TO 10
        LPRINT USING "Num ## : ###"; i; outpt1(j);
        LPRINT USING "  Num ## : ###"; i + 10; outpt1(j + 10);
        LPRINT USING "  Num ## : ###"; i + 20; outpt1(j + 20);
        LPRINT USING "  Num ## : ###"; i + 30; outpt1(j + 30);
        LPRINT USING "  Num ## : ###"; i + 40; outpt1(j + 40)
    NEXT i
    LPRINT
    LPRINT
    ...
    LPRINT
    FOR i = 1 TO 10
        LPRINT USING "Num ## : ###"; i; outpt2(i);
        LPRINT USING "  Num ## : ###"; i + 10; outpt2(j + 10);
        LPRINT USING "  Num ## : ###"; i + 20; outpt2(j + 20);
        LPRINT USING "  Num ## : ###"; i + 30; outpt2(j + 30);
        LPRINT USING "  Num ## : ###"; i + 40; outpt2(j + 40)
    NEXT i
    LPRINT
    LPRINT "Totals of all numbers in draws : "; total
    LPRINT "Average numbers drawn (total/50 rounded off : "; average
    LPRINT "Low number thresh hold : "; low
    LPRINT "High Number thresh hold : "; high
...
ELSEIF answer$ = "l" OR answer$ = "L" THEN
    CLS
    counter = 1
    PRINT "Toggle Printer On or Off   > Printer is : ";: IF hardcopy$ = "Y" THEN PRINT "ON";: IF hardcopy$ = "N" THEN PRINT "OFF"
    PRINT
...
FOR i = 1 TO amount
    ...
    IF crs$ = "Y" THEN COLOR 10, 5
    LOCATE 4, 11
    PRINT "   Lotto 649 Package Version 2020 RC4 Search and Print Menu   "
    IF crs$ = "Y" THEN COLOR 4, 7
    FOR i = 6 TO 10
        LOCATE i, 17
        PRINT "["
...
sc:
szc:
za = entry(1): zb = entry(2): zc = entry(3): zd = entry(4): ze = entry(5): zf = entry(6): zg = entry(7): zh = entry(8): zi = entry(9): zj = entry(10): zk = entry(11): zl = entry(12): zm = entry(13): zn = entry(14): zo = entry(15)
...
FOR i = 1 TO linecounter
    ...
    FOR j = 4 TO 11
        IF chart(i, j) = za OR chart(i, j) = zb OR chart(i, j) = zc OR chart(i, j) = zd OR chart(i, j) = ze OR chart(i, j) = zf OR chart(i, j) = zg THEN
            match = match + 1
        END IF
        IF chart(i, j) = zh AND match = 5 THEN
            match = match + 1
        END IF
    NEXT j
    IF match > 2 THEN
...
DO UNTIL EOF(2)
    ...
    IF e = ac OR e = bc OR e = cc OR e = dc OR e = ec OR e = fc OR e = gc THEN
        winnings = winnings + 1
    END IF
    IF f = ac OR f = bc OR f = cc OR f = dc OR f = ec OR f = fc OR e = gc THEN
        winnings = winnings + 1
    END IF
    ...
    END IF
    IF winnings = 7 THEN
        IF h = ac OR h = bc OR h = cc OR h = dc OR h = ec OR h = fc OR h = gc THEN
            winnings = winnings + 1
        END IF
    END IF
    ...
    END IF
    IF winnings = 6 THEN
        PRINT "You have 6 corrrect numbers in line # "; linecount
    END IF
    IF winnings = 7 THEN
...
CLS
PRINT
PRINT "Lotto 649 Package Version 2020 RC4 Winning Numbers Entry"
PRINT "--------------------------------------------------------"
PRINT
...
count = 0
GOSUB calculatedayofweek
PRINT "Winning Numbers Updated on "; dayofweek$(day1); " "; month$(m); " "; dy; " "; y
PRINT "Last numbers entered : "; a; b; c; d; e; f; g, h



PRINT
PRINT "Next Draw Date "; dayofweek$(day8); " "; month$(m); " "; dy; " "; y
PRINT
...
IF a = 0 THEN CLOSE #1: count = -1: GOSUB add3or4days: GOTO main
...
PRINT
PRINT "You Have Entered for : "; dayofweek$(day1); " "; month$(m); " "; dy; " "; y
...
IF answer$ = "n" OR answer$ = "N" OR answer$ = "no" OR answer$ = "NO" THEN count = -1: GOSUB add3or4days: GOTO beginning
IF answer$ = "Q" OR answer$ = "q" THEN CLOSE #1: GOTO main
IF answer$ = "Y" THEN

    PRINT #1, USING "## ## #### ## ## ## ## ## ## ## ## "; m; dy; y; a; b; c; d; e; f; g; h
...
FOR i = 1 TO linecounter
    ...
    numb(chart(i, 11)) = bonus(chart(i, 11)) + 1
...
LOCATE 4, 18
PRINT "Lotto 649 Version 2020 RC4 Print Menu"
IF crs$ = "Y" THEN COLOR 3, 8
LOCATE 3, 17
PRINT CHR$(201)
FOR i = 18 TO 57
...
FOR i = 1 TO linecounter
    ...
    FOR zz = 1 TO amount00000: NEXT zz
    qzqzqz:
NEXT i
DO WHILE INKEY$ = ""
LOOP
